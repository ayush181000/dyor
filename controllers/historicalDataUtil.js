...
const historicalStaticData = async (tokenName) => {
  ...
  let latestActiveHolders = latestData[0]?.activeHolders || null;
  ...
  const {
    price: price60d,
    circulatingSupply: circulatingSupply60d,
    tvl: tvl60d,
    holders: holders60d,
    activeHolders: activeHolders60d,
  } = (await TokenData.findOne({ tokenName, date: getOlderDate("60d") })) ||
  nullObject;
  const {
    price: price90d,
    circulatingSupply: circulatingSupply90d,
    tvl: tvl90d,
    holders: holders90d,
    activeHolders: activeHolders90d,
  } = (await TokenData.findOne({ tokenName, date: getOlderDate("90d") })) ||
  nullObject;
  const {
    price: price365d,
    circulatingSupply: circulatingSupply365d,
    tvl: tvl365d,
    holders: holders365d,
    activeHolders: activeHolders365d,
  } = (await TokenData.findOne({ tokenName, date: getOlderDate("365d") })) ||
  nullObject;

  const tokenTradingVolumeMetric = await tokenTradingVolMetricFunc(tokenName);
  const feeMetric = await feeMetricFunc(tokenName);
  if (tokenName === "Chainlink") {
      latestActiveHolders = null;
  }
  const fair_price = fairPriceCalculation(
    latestTvl === null || latestTvl === 0
      ? null
      : percChange(latestTvl, tvl90d),
    tokenTradingVolumeMetric.percChange90d,
    feeMetric.percChange90d,
    latestActiveHolders === null || latestActiveHolders === 0
      ? null
      : percChange(latestActiveHolders, activeHolders90d),
    latestHolders === null || latestHolders === 0
      ? null
      : percChange(latestHolders, holders90d),
    latestCirculatingSupply === null || latestCirculatingSupply === 0
      ? null
...
const fairPriceCalculation = (
  ...
  const fair_price_percentage =
    average_demand_change_perc - average_supply_change_perc - pricePerc;
...
const dataFallback = async (chainNames) => {
  ...
  for (let chain of chainNames) {
    ...
    const result = await TokenData.find({ tokenName: chain })
      .sort({ date: -1 })
      .limit(7);
    // console.log(result);
    ...
    for (let i = 0; i < result.length; i++) {
      ...
      tempData = {
        ...
        tvl:
          tempData["tvl"] == null && _doc["tvl"] && _doc["tvl"] > 0
            ? _doc["tvl"]
            : tempData["tvl"],

        fdv:
          tempData["fdv"] == null && _doc["fdv"] && _doc["fdv"] > 0
            ? _doc["fdv"]
            : tempData["fdv"],

        holders:
          tempData["holders"] == null && _doc["holders"] && _doc["holders"] > 0
            ? _doc["holders"]
            : tempData["holders"],

        activeHolders:
          tempData["activeHolders"] == null &&
          _doc["activeHolders"] &&
          _doc["activeHolders"] > 0
            ? _doc["activeHolders"]
            : tempData["activeHolders"],

        totalSupply:
          tempData["totalSupply"] == null &&
          _doc["totalSupply"] &&
          _doc["totalSupply"] > 0
            ? _doc["totalSupply"]
            : tempData["totalSupply"],

        daily_fee:
          tempData["daily_fee"] == null &&
          _doc["daily_fee"] &&
          _doc["daily_fee"] > 0
            ? _doc["daily_fee"]
            : tempData["daily_fee"],
        ...
        price:
          tempData["price"] == null && _doc["price"] && _doc["price"] > 0
            ? _doc["price"]
            : tempData["price"],

        ttv:
          tempData["ttv"] == null && _doc["ttv"] && _doc["ttv"] > 0
            ? _doc["ttv"]
            : tempData["ttv"],

        circulatingSupply:
          tempData["circulatingSupply"] == null &&
          _doc["circulatingSupply"] &&
          _doc["circulatingSupply"] > 0
            ? _doc["circulatingSupply"]
            : tempData["circulatingSupply"],

        stakingRatio:
          tempData["stakingRatio"] == null &&
          _doc["stakingRatio"] &&
          _doc["stakingRatio"] > 0
            ? _doc["stakingRatio"]
            : tempData["stakingRatio"],

        marketCap:
          tempData["marketCap"] == null &&
          _doc["marketCap"] &&
          _doc["marketCap"] > 0
            ? _doc["marketCap"]
            : tempData["marketCap"],
