...
async function getHistoricalTVL(){
    try {
        const res = await axios.get(
          `${DEFI_LLAMA_URL}v2/historicalChainTvl/Arbitrum`
        );
...
async function getMonthlyTVL() {
    const data = await getHistoricalTVL();
  const groupedData = {};
  data.forEach(item => {
...
async function get60DaysTVL() {
  ...
  for (const item of data) {
    ...
    while (
      currentDate - currentIntervalStart >=
      60 * 24 * 60 * 60 * 1000
    ) {
...
module.exports = {getMonthlyTVL , getYearlyTVL , getQuaterlyTVL , get60DaysTVL};